...
import {
  ...
  Wallet, User, Phone, Mail, Globe, Activity, ShieldCheck, CreditCard, Settings, ArrowDown
} from "lucide-react";
...
export default function App() {
  ...
  useEffect(() => {
    const titleMap = {
      estimator: "Hospital Project Estimator | Innovate IndAI",
      architect: "AI Hospital Layout Generator | Innovate IndAI",
      boq: "Smart BOQ Calculator | Innovate IndAI",
      compliance: "NABH Compliance Checker | Innovate IndAI",
      funding: "Hospital Loan Syndication | Innovate IndAI",
      dpr: "Generate Hospital DPR | Innovate IndAI",
      vendors: "B2B Consortium | Innovate IndAI",
      admin: "Admin Dashboard | Innovate IndAI",
      contact: "Contact Hospital Consultants | Innovate IndAI"
    };
    document.title = titleMap[activeTab] || "Innovate IndAI Hospital Consultancy";
  ...
  const engine = useMemo(() => {
    try {
      ...
      const minAreaPerBed = tConf.areaPerBed * 0.85; // 85% rule for NABH
      const nabhReady = sqFtPerBed >= minAreaPerBed;
      const maturityScore = Math.min(100, (sqFtPerBed / tConf.areaPerBed) * 100);
      
      const areaShortfall = nabhReady ? 0 : (b * minAreaPerBed) - a;
      ...
      return { type: "Error", sqFtPerBed: 0, minAreaPerBed: 0, nabhReady: false, maturityScore: 0, areaShortfall: 0, nursingStaff: 0, residentDoctors: 0, ipdArea: 0, icuOtArea: 0, totalProjectCost: 0, totalRevenue: 0, ebitda: 0, ebitdaMargin: 0, dscr: 0, breakEvenYears: 0, loanAmount: 0, equity: 0, annualEMI: 0 };
    }
  }, [config]);
  ...
  const handleLeadSubmit = async () => {
    if (!clientDetails.name || !clientDetails.phone) return alert("Please enter Name and WhatsApp number.");
    setIsSyncing(true);
    try { 
      await supabase.from('projects').insert([{ 
        project_name: clientDetails.projectName || "Lead Inquiry", director_email: clientDetails.email || "No Email", client_name: clientDetails.name, client_phone: clientDetails.phone, beds_count: config.beds, total_capex: engine.totalProjectCost, hospital_type: engine.type 
      }]); 
    } catch(e) { console.error(e); }
    try {
      await fetch('https://api.resend.com/emails', {
        method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${RESEND_API_KEY}` },
        body: JSON.stringify({ from: `Innovate IndAI Lead <${USER_CONFIG.reportEmail}>`, to: [USER_CONFIG.email], subject: `NEW LEAD: ${leadActionType} | ${clientDetails.name}`, html: `<h3>New Lead</h3><p>Action: ${leadActionType}</p><p>Name: ${clientDetails.name}</p><p>Phone: ${clientDetails.phone}</p><p>Scale: ${config.beds} Beds</p>` })
      });
    } catch (e) { console.error(e); }
    setIsSyncing(false); setShowLeadModal(false);
    alert(`Success! Your request for "${leadActionType}" has been received.`);
  };
  ...
  const handleDPRCheckout = async () => {
    ...
    try {
      const response = await fetch("https://innovate-india-suite.onrender.com/api/admin/generate-pdf", {
        ...
        body: JSON.stringify({ email: clientDetails.email, projectName: clientDetails.projectName || "New Hospital Project", bedCount: config.beds, specialtyFocus: engine.type, cityTier: config.cityTier, totalArea: config.areaSqFt, numFloors: archInputs.floors || "4" })
      });
      ...
      if (result.success && result.pdfBase64) {
        ...
        downloadLink.download = `${(clientDetails.projectName || "Innovate_IndAI").replace(/[^a-z0-9]/gi, '_')}_DPR_Report.pdf`;
        downloadLink.click();
        alert("Success! Your detailed DPR Report has been downloaded to your device.");
      } else { alert("Failed to generate PDF. Check server logs."); }
    } catch (e) { alert("Could not connect to the backend engine."); }
    setIsSyncing(false);
  };

  const handlePromoApply = () => {
    if (pricingState.promoCode.toUpperCase() === "WELCOME50") {
      setPricingState({ ...pricingState, discountApplied: true }); alert("50% Early Adopter Discount Applied!");
    } else { setPricingState({ ...pricingState, discountApplied: false }); alert("Invalid Promo Code."); }
  };
  ...
  const handleVendorApply = async () => {
    ...
    try {
      await fetch('https://api.resend.com/emails', {
        method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${RESEND_API_KEY}` },
        body: JSON.stringify({ from: `Innovate IndAI <${USER_CONFIG.reportEmail}>`, to: [USER_CONFIG.email], subject: `New Vendor | ${vendorDetails.company}`, html: `<p>Firm: ${vendorDetails.company}</p><p>Cat: ${vendorDetails.category}</p><p>Ph: ${vendorDetails.contact}</p>` })
      });
    ...
    <div className="flex h-screen w-full bg-[#010810] text-white font-sans selection:bg-[#D4AF37] selection:text-[#010810] overflow-hidden">
          ...
          <div className="bg-[#0A2540] border border-[#D4AF37]/50 rounded-[30px] p-8 md:p-10 w-full max-w-md shadow-[0_0_40px_rgba(212,175,55,0.2)] relative">
            ...
            <div className="space-y-4">
              <div className="flex items-center border-b border-white/20 pb-2">
                <User className="w-5 h-5 text-[#D4AF37] mr-3" /><input placeholder="Your Full Name*" value={clientDetails.name} onChange={e=>setClientDetails({...clientDetails, name: e.target.value})} className="w-full bg-transparent outline-none text-white font-bold placeholder-white/30" />
              </div>
              <div className="flex items-center border-b border-white/20 pb-2">
                <Phone className="w-5 h-5 text-[#D4AF37] mr-3" /><input type="tel" placeholder="WhatsApp Number*" value={clientDetails.phone} onChange={e=>setClientDetails({...clientDetails, phone: e.target.value})} className="w-full bg-transparent outline-none text-white font-bold placeholder-white/30" />
              </div>
              <div className="flex items-center border-b border-white/20 pb-2">
                <Mail className="w-5 h-5 text-[#D4AF37] mr-3" /><input type="email" placeholder="Email Address (Optional)" value={clientDetails.email} onChange={e=>setClientDetails({...clientDetails, email: e.target.value})} className="w-full bg-transparent outline-none text-white font-bold placeholder-white/30" />
              </div>
      ...
      <aside className={`fixed inset-y-0 left-0 z-40 transform ${isMobileMenuOpen ? "translate-x-0" : "-translate-x-full"} md:relative md:translate-x-0 transition duration-300 ease-in-out w-72 bg-[#051626] border-r border-white/5 flex flex-col flex-shrink-0 shadow-2xl`}>
        <div className="p-6 border-b border-white/5 flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-[#D4AF37] to-[#F1CF6D] flex items-center justify-center text-[#0A2540] shadow-[0_0_15px_rgba(212,175,55,0.4)] shrink-0">
            <Building2 className="w-6 h-6" />
          </div>
          <div className="flex flex-col">
            <a href="/" style={{ fontFamily: "'Montserrat', sans-serif" }} className="text-xl font-black tracking-widest uppercase text-white leading-none flex items-baseline mt-1">
                INNOVATE <span style={{ color: '#FBC02D', marginLeft: '4px' }}>INDAI</span>
            </a>
            <span className="text-[7px] text-gray-400 uppercase tracking-[0.2em] mt-1.5 font-semibold leading-none">
        ...
        <nav className="flex-1 overflow-y-auto p-4 space-y-2 custom-scrollbar">
          {navItems.map((item) => {
            const NavIcon = item.icon; const isActive = activeTab === item.id;
            return (
              <button key={item.id} onClick={() => { setActiveTab(item.id); window.location.hash = `/${item.id}`; setIsMobileMenuOpen(false); }} 
                ...
                <NavIcon className="w-5 h-5 shrink-0" /><span>{item.label}</span>
              </button>
            );
          })}
        </nav>
          ...
          <div className="flex-1 p-6 md:p-10 max-w-[1400px] w-full mx-auto animate-in fade-in duration-500 relative z-10">
            ...
            {activeTab === "estimator" && (
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
                <div className={GLOW_CARD}>
                  <div className="mb-6">
                    <label className="text-[10px] text-[#D4AF37] font-black flex justify-between mb-3 uppercase tracking-widest"><span>Scale</span><span className="text-white">{config.beds} Beds</span></label>
                  ...
                  <div className="mb-6">
                    <label className="text-[10px] text-[#D4AF37] font-black flex justify-between mb-3 uppercase tracking-widest"><span>Occupancy</span><span className="text-white">{config.occupancyRate}%</span></label>
                  ...
                  <div className={`transition-all duration-500 ${!engine.nabhReady ? 'mb-4 p-4 -mx-4 border border-red-500/40 bg-red-500/5 rounded-xl shadow-[0_0_15px_rgba(239,68,68,0.1)]' : 'mb-6'}`}>
                    <label className="text-[10px] text-[#D4AF37] font-black flex justify-between mb-3 uppercase tracking-widest">
                      <span className={!engine.nabhReady ? "text-red-400" : ""}>Total Area</span>
                      <span className={!engine.nabhReady ? "text-red-400 font-black" : "text-white"}>{config.areaSqFt.toLocaleString()} Sqft</span>
                    </label>
                    <input type="range" min="20000" max="400000" step="5000" value={config.areaSqFt} onChange={e=>setConfig({...config, areaSqFt: Number(e.target.value)})} className={`w-full ${!engine.nabhReady ? 'accent-red-500' : 'accent-[#D4AF37]'}`} />
                    {!engine.nabhReady && (
                      <div className="mt-4 pt-4 border-t border-red-500/20 animate-in fade-in slide-in-from-top-2 duration-300">
                        <p className="text-[10px] font-black text-red-400 uppercase tracking-widest flex items-center gap-1.5 mb-1.5">
                          <AlertCircle className="w-3.5 h-3.5" /> NABH Space Deficit
                        </p>
                        <p className="text-[10px] text-red-200/80 leading-relaxed">
                          Current area provides <strong>{Math.round(engine.sqFtPerBed)} sq.ft/bed</strong>. 
                          NABH requires ~<strong>{Math.round(engine.minAreaPerBed)} sq.ft/bed</strong> for a {engine.type} setup. 
                          Increase total area by <strong>{Math.ceil(engine.areaShortfall).toLocaleString()} sq.ft</strong> or reduce your bed count to comply.
                        </p>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="pt-6 mt-6 border-t border-white/5">
                    <p className="text-[10px] text-white/50 uppercase font-black tracking-widest mb-3">Investor Readiness Gauge</p>
                    <div className="w-full bg-[#010810] h-3 rounded-full overflow-hidden border border-white/5">
                      <div className={`h-full transition-all duration-500 ${!engine.nabhReady ? 'bg-gradient-to-r from-red-600 to-red-400' : 'bg-gradient-to-r from-[#D4AF37] to-[#F1CF6D]'}`} style={{width: `${engine.maturityScore}%`}}></div>
                    </div>
                  </div>
                </div>
              ...
              <div className="space-y-6 md:space-y-8">
                ...
                <div className={GLOW_CARD}>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div><label className="text-[10px] text-white/50 uppercase font-black mb-2 block tracking-widest">Land Area (Sq.Ft)</label><input type="number" value={archInputs.landArea || ""} onChange={e=>setArchInputs({...archInputs, landArea: e.target.value})} className="w-full bg-[#010810] border border-white/10 p-4 rounded-xl text-white font-bold outline-none focus:border-[#D4AF37] text-sm" placeholder="e.g. 50000" /></div>
                    <div><label className="text-[10px] text-white/50 uppercase font-black mb-2 block tracking-widest">Bed Capacity</label><input type="number" value={config.beds || ""} onChange={e=>setConfig({...config, beds: Number(e.target.value)})} className="w-full bg-[#010810] border border-white/10 p-4 rounded-xl text-white font-bold outline-none focus:border-[#D4AF37] text-sm" /></div>
                    <div><label className="text-[10px] text-white/50 uppercase font-black mb-2 block tracking-widest">Number of Floors</label><input type="number" value={archInputs.floors || ""} onChange={e=>setArchInputs({...archInputs, floors: e.target.value})} className="w-full bg-[#010810] border border-white/10 p-4 rounded-xl text-white font-bold outline-none focus:border-[#D4AF37] text-sm" /></div>
                    <div><label className="text-[10px] text-white/50 uppercase font-black mb-2 block tracking-widest">City & Local By-laws</label><select className="w-full bg-[#010810] border border-white/10 p-4 rounded-xl text-white font-bold outline-none focus:border-[#D4AF37] text-sm"><option>Surat (GDCR Rules)</option><option>Ahmedabad (AUDA Rules)</option><option>Standard National Guidelines</option></select></div>
                    <div className="md:col-span-2"><label className="text-[10px] text-white/50 uppercase font-black mb-2 block tracking-widest">Required Specialties & Departments</label><input type="text" value={archInputs.specialties || ""} onChange={e=>setArchInputs({...archInputs, specialties: e.target.value})} className="w-full bg-[#010810] border border-white/10 p-4 rounded-xl text-white font-bold outline-none focus:border-[#D4AF37] text-sm" /></div>
                    <div className="md:col-span-2"><label className="text-[10px] text-white/50 uppercase font-black mb-2 block tracking-widest">Parking Requirements</label><select value={archInputs.parking || ""} onChange={e=>setArchInputs({...archInputs, parking: e.target.value})} className="w-full bg-[#010810] border border-white/10 p-4 rounded-xl text-white font-bold outline-none focus:border-[#D4AF37] text-sm"><option>Basement + Surface Parking (Standard)</option><option>Double Basement</option><option>Surface Parking Only</option><option>Mechanized / Stack Parking</option></select></div>
                  </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                   <div className={GLOW_CARD}>
                      ...
                      <div className="space-y-4">
                        {[{r: "OPD near entrance", p: "Easy access for outpatients, reduces congestion."}, {r: "ER accessible from road", p: "Quick ambulance entry, bypasses public areas."}, {r: "ICU adjacent to OT", p: "Immediate transfer of critical post-op patients."}, {r: "Wards in quiet zones", p: "Ensures rest, minimizes noise & infection risk."}, {r: "Service corridors separate", p: "Prevents overlap of patient, staff & waste flows."}].map((rule, idx) => (
                   ...
                   <div className="space-y-6">
                     ...
                     <div className="bg-amber-500/10 border border-amber-500/30 p-6 md:p-8 rounded-2xl md:rounded-[30px] flex items-start gap-4 shadow-xl">
                       ...
                       <div><p className="font-black text-amber-500 uppercase tracking-widest text-sm mb-2">100% NABH Compliant</p><p className="text-xs text-amber-100/70 leading-relaxed">The AI encodes statutory regulations into every decision, ensuring the generated floor plans pass medical board inspections.</p></div>
                     </div>
                   </div>
                </div>
              </div>
            ...
            {activeTab === "boq" && (
              <div className="space-y-6 md:space-y-8">
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 md:gap-6">
                   <div className={`${GLOW_CARD} !p-6 bg-gradient-to-br from-[#0A2540] to-[#051626]`}><p className="text-[10px] text-white/50 uppercase font-black mb-2 tracking-widest">Total BOQ Value</p><p className="text-3xl lg:text-4xl font-black text-[#D4AF37]">{(formatINR(boqData.grandTotal))}</p></div>
                   <div className={`${GLOW_CARD} !p-6`}><p className="text-[10px] text-white/50 uppercase font-black mb-2 tracking-widest">Cost Per Sq.Ft</p><p className="text-3xl lg:text-4xl font-black text-white">₹{safeToFixed(boqData.grandTotal / Math.max(1, config.areaSqFt), 0)}</p></div>
                   <div className={`${GLOW_CARD} !p-6 flex flex-col justify-center items-start`}><p className="text-[10px] text-white/50 uppercase font-black mb-4 tracking-widest">Export Detailed BOQ</p><div className="flex gap-3 w-full"><button onClick={() => triggerLeadCapture("Excel BOQ Export")} className="flex-1 flex items-center justify-center gap-2 bg-emerald-600/20 text-emerald-400 border border-emerald-500/50 py-3 rounded-lg font-bold text-xs uppercase tracking-widest hover:bg-emerald-600 hover:text-white transition-colors"><Download className="w-4 h-4"/> Excel</button><button onClick={() => triggerLeadCapture("PDF BOQ Export")} className="flex-1 flex items-center justify-center gap-2 bg-[#D4AF37]/20 text-[#D4AF37] border border-[#D4AF37]/50 py-3 rounded-lg font-bold text-xs uppercase tracking-widest hover:bg-[#D4AF37] hover:text-[#051626] transition-colors"><Download className="w-4 h-4"/> PDF</button></div></div>
                </div>
                ...
                <div className="space-y-6">
                  {boqData.categories.filter(c => activeBoqCategory === "all" || activeBoqCategory === c.id).map(category => (
                    <div key={category.id} className="bg-[#0A2540]/50 backdrop-blur-md rounded-2xl md:rounded-[30px] border border-white/10 overflow-hidden shadow-xl">
                      <div className="bg-[#051626] p-5 md:p-6 border-b border-white/10 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2"><h3 className="font-black text-[#D4AF37] text-sm md:text-base uppercase tracking-widest">{category.name}</h3><span className="font-black text-white text-lg">₹{category.catTotal.toLocaleString('en-IN')}</span></div>
                      <div className="overflow-x-auto">
                        <table className="w-full text-left border-collapse min-w-[600px]">
                          <thead><tr className="bg-white/5 text-white/40 uppercase text-[10px] tracking-widest"><th className="p-4 md:p-5 font-bold">Item Description</th><th className="p-4 md:p-5 font-bold">Unit</th><th className="p-4 md:p-5 font-bold text-right">Qty</th><th className="p-4 md:p-5 font-bold text-right">Rate (₹)</th><th className="p-4 md:p-5 font-bold text-right text-[#D4AF37]">Amount (₹)</th></tr></thead>
                          <tbody className="text-xs md:text-sm text-white/80 divide-y divide-white/5">{category.items.map((item, idx) => (<tr key={idx} className="hover:bg-white/5 transition-colors"><td className="p-4 md:p-5 font-medium">{item.desc}</td><td className="p-4 md:p-5 text-white/50">{item.unit}</td><td className="p-4 md:p-5 text-right">{item.qty.toLocaleString()}</td><td className="p-4 md:p-5 text-right">{item.rate.toLocaleString('en-IN')}</td><td className="p-4 md:p-5 text-right font-black text-white">{(item.amount).toLocaleString('en-IN')}</td></tr>))}</tbody>
                        </table>
                      </div>
                    </div>
                  ))}
            ...
            {activeTab === "compliance" && (
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8">
                <div className={GLOW_CARD}><h3 className="text-xl md:text-2xl font-black mb-8 text-[#D4AF37] flex items-center gap-3"><ClipboardCheck/> Mandatory Approvals</h3><div className="space-y-4"><div className="flex items-center gap-4 p-4 md:p-5 bg-[#010810] border border-white/5 rounded-xl"><CheckCircle2 className="w-6 h-6 text-[#4ade80] shrink-0"/><span className="font-bold text-xs md:text-sm">Clinical Establishment Registration</span></div><div className="flex items-center gap-4 p-4 md:p-5 bg-[#010810] border border-white/5 rounded-xl"><CheckCircle2 className="w-6 h-6 text-[#4ade80] shrink-0"/><span className="font-bold text-xs md:text-sm">Fire Department NOC</span></div><div className="flex items-center gap-4 p-4 md:p-5 bg-[#010810] border border-white/5 rounded-xl"><CheckCircle2 className="w-6 h-6 text-[#4ade80] shrink-0"/><span className="font-bold text-xs md:text-sm">Gujarat Pollution Control Board (GPCB)</span></div><div className="flex items-center gap-4 p-4 md:p-5 bg-[#010810] border border-white/5 rounded-xl"><CheckCircle2 className="w-6 h-6 text-[#4ade80] shrink-0"/><span className="font-bold text-xs md:text-sm">Bio-Medical Waste Authorization</span></div></div></div>
                <div className={GLOW_CARD}><h3 className="text-xl md:text-2xl font-black mb-8 text-[#D4AF37] flex items-center gap-3"><ShieldCheck/> Department Licenses</h3><div className="space-y-4"><div className="flex items-center gap-4 p-4 md:p-5 bg-[#010810] border border-white/5 rounded-xl"><AlertCircle className="w-6 h-6 text-[#D4AF37] shrink-0"/><span className="font-bold text-xs md:text-sm">AERB Approval (Atomic Energy - X-Ray/CT)</span></div><div className="flex items-center gap-4 p-4 md:p-5 bg-[#010810] border border-white/5 rounded-xl"><AlertCircle className="w-6 h-6 text-[#D4AF37] shrink-0"/><span className="font-bold text-xs md:text-sm">PNDT Act Registration (Ultrasound)</span></div><div className="flex items-center gap-4 p-4 md:p-5 bg-[#010810] border border-white/5 rounded-xl"><AlertCircle className="w-6 h-6 text-[#D4AF37] shrink-0"/><span className="font-bold text-xs md:text-sm">Pharmacy License (State FDA)</span></div><div className="flex items-center gap-4 p-4 md:p-5 bg-[#010810] border border-white/5 rounded-xl"><AlertCircle className="w-6 h-6 text-[#D4AF37] shrink-0"/><span className="font-bold text-xs md:text-sm">Local Authority (PCPIR / GIDC / AUDA)</span></div></div></div>
              </div>
            )}
            ...
            {activeTab === "vendors" && (
              <div className="space-y-8 md:space-y-10 animate-in slide-in-from-bottom duration-700">
                ...
                <div className={`${GLOW_CARD} flex flex-col md:flex-row items-start md:items-center gap-6 md:gap-10 !p-6 md:!p-10`}>
                  <div className="flex-1"><h2 className="text-3xl md:text-4xl font-black mb-2">Join Network</h2><p className="text-[#D4AF37] text-xs md:text-sm">Commission-based lead distribution for OEMs and Consultants.</p></div>
                  <div className="flex-1 space-y-3 md:space-y-4 w-full">
                    ...
                    <select value={vendorDetails.category || ""} onChange={e => setVendorDetails({...vendorDetails, category: e.target.value})} className="w-full bg-[#051626] border border-white/10 p-3 rounded-lg text-white/70 font-bold outline-none focus:border-[#D4AF37] text-sm md:text-base"><option>Hospital Planning Consultant</option><option>Healthcare Architect</option><option>MEP Consultant</option><option>Medical Equipment Vendor</option><option>Financial Institution</option></select>
                  ...
                  <div className="space-y-3 md:space-y-4">
                    {PACKAGES.map(pkg => (
                      <button key={pkg.id} onClick={() => setPricingState({ packageId: pkg.id, promoCode: "", discountApplied: false })} className={`w-full text-left p-4 md:p-5 rounded-xl md:rounded-2xl border-2 transition-all duration-300 ${pricingState.packageId === pkg.id ? 'border-[#D4AF37] bg-[#D4AF37]/10 shadow-[0_0_15px_rgba(212,175,55,.2)]' : 'border-white/5 bg-[#051626] hover:border-white/20'}`}>
                        <div className="flex justify-between items-center mb-1"><span className="font-bold text-sm md:text-lg">{pkg.name}</span><span className={`font-black text-base md:text-xl ${pricingState.packageId === pkg.id ? 'text-[#D4AF37]' : 'text-emerald-400'}`}>{formatPrice(pkg.price)}</span></div>
                ...
                <div className="bg-white text-[#0A2540] rounded-2xl md:rounded-[30px] p-6 md:p-10 shadow-2xl flex flex-col justify-between">
                  <div className="space-y-4 md:space-y-5">
                    ...
                    <div className="flex items-center border-b-2 border-gray-200 focus-within:border-[#D4AF37] pb-2">
                      <User className="w-4 h-4 md:w-5 md:h-5 text-gray-400 mr-2 md:mr-3"/><input placeholder="Full Name" value={clientDetails.name || ""} onChange={e=>setClientDetails({...clientDetails, name: e.target.value})} className="w-full outline-none text-base md:text-lg font-bold bg-transparent text-[#0A2540]"/>
                    </div>
                    <div className="flex items-center border-b-2 border-gray-200 focus-within:border-[#D4AF37] pb-2 mt-4 md:mt-5">
                      <Phone className="w-4 h-4 md:w-5 md:h-5 text-gray-400 mr-2 md:mr-3"/><input placeholder="WhatsApp Number" value={clientDetails.phone || ""} onChange={e=>setClientDetails({...clientDetails, phone: e.target.value})} className="w-full outline-none text-base md:text-lg font-bold bg-transparent text-[#0A2540]"/>
                    </div>
                    <div className="flex items-center border-b-2 border-gray-200 focus-within:border-[#D4AF37] pb-2 mt-4 md:mt-5">
                      <Mail className="w-4 h-4 md:w-5 md:h-5 text-gray-400 mr-2 md:mr-3"/><input placeholder="Email Address" value={clientDetails.email || ""} onChange={e=>setClientDetails({...clientDetails, email: e.target.value})} className="w-full outline-none text-base md:text-lg font-bold bg-transparent text-[#0A2540]"/>
                    </div>
                    <div className="grid grid-cols-2 gap-4 md:gap-6 pt-4 md:pt-5">
                      <div className="flex items-center border-b-2 border-gray-200 focus-within:border-[#D4AF37] pb-2">
                        <MapPin className="w-4 h-4 md:w-5 md:h-5 text-gray-400 mr-2 md:mr-3"/><input placeholder="City" value={clientDetails.city || ""} onChange={e=>setClientDetails({...clientDetails, city: e.target.value})} className="w-full outline-none text-sm md:text-lg font-bold bg-transparent text-[#0A2540]"/>
                      </div>
                      <div className="flex items-center border-b-2 border-gray-200 focus-within:border-[#D4AF37] pb-2">
                        <Globe className="w-4 h-4 md:w-5 md:h-5 text-gray-400 mr-2 md:mr-3"/><input placeholder="State" value={clientDetails.state || ""} onChange={e=>setClientDetails({...clientDetails, state: e.target.value})} className="w-full outline-none text-sm md:text-lg font-bold bg-transparent text-[#0A2540]"/>
                      </div>
                    </div>
                    <div className="flex items-center border-b-2 border-gray-200 focus-within:border-[#D4AF37] pb-2 mt-4 md:mt-5">
                      <Building2 className="w-4 h-4 md:w-5 md:h-5 text-gray-400 mr-2 md:mr-3"/><input placeholder="Project Name" value={clientDetails.projectName || ""} onChange={e=>setClientDetails({...clientDetails, projectName: e.target.value})} className="w-full outline-none text-base md:text-lg font-bold bg-transparent text-[#0A2540]"/>
                    </div>
                  ...
                  <div className="mt-8 pt-6 border-t-2 border-dashed border-gray-200">
                    ...
                    <button onClick={handleDPRCheckout} disabled={isSyncing} className="w-full bg-[#0A2540] text-[#D4AF37] py-4 md:py-5 rounded-xl md:rounded-2xl font-black uppercase tracking-widest flex justify-center items-center gap-3 text-sm md:text-base hover:scale-[1.02] transition-transform overflow-hidden relative">
                      {isSyncing ? (<><Activity className="w-5 h-5 animate-spin shrink-0"/><span className="animate-pulse whitespace-nowrap">{loadingMessages[loadingIndex]}</span></>) : (pricingState.packageId === "free" ? "Claim Free Brief" : "Secure Checkout")}
                    </button>
                  </div>
                </div>
              </div>
            ...
            {activeTab === "admin" && (
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 md:gap-8 animate-in fade-in duration-500">
                <div className={GLOW_CARD}><p className="text-[10px] md:text-xs uppercase text-[#D4AF37] font-black tracking-widest mb-2 md:mb-3">DPR Reports Generated</p><p className="text-4xl md:text-6xl font-black text-white">{adminProjects.length}</p></div>
                <div className={GLOW_CARD}><p className="text-[10px] md:text-xs uppercase text-[#D4AF37] font-black tracking-widest mb-2 md:mb-3">Enrolled Network Vendors</p><p className="text-4xl md:text-6xl font-black text-white">{adminVendors.length}</p></div>
                <div className={GLOW_CARD}><p className="text-[10px] md:text-xs uppercase text-[#D4AF37] font-black tracking-widest mb-2 md:mb-3">Platform Revenue</p><p className="text-4xl md:text-6xl font-black text-[#4ade80]">₹0</p></div>
              </div>
            )}
            ...
            <div className="max-w-[1200px] mx-auto grid grid-cols-1 md:grid-cols-4 gap-8 mb-8">
              <div className="md:col-span-2">
                <div className="flex items-center justify-center md:justify-start gap-2 mb-4">
                  ...
                  <a href="/" style={{ fontFamily: "'Montserrat', sans-serif" }} className="font-black text-lg tracking-widest uppercase text-white flex items-baseline">
                      INNOVATE <span style={{ color: '#FBC02D', marginLeft: '4px' }}>INDAI</span>
                  </a>
                </div>
